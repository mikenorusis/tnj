...
impl Database {
    ...
    pub fn insert_task(&self, task: &Task) -> Result<i64, DatabaseError> {
        self.conn.execute(
            "INSERT INTO tasks (title, description, due_date, status, tags, \"order\", archived, created_at, updated_at)
    ...
    pub fn insert_journal(&self, journal: &JournalEntry) -> Result<i64, DatabaseError> {
        self.conn.execute(
            "INSERT INTO journals (date, title, content, tags, archived, created_at, updated_at)
    ...
    /// Get all tasks ordered by order ASC
    pub fn get_all_tasks(&self) -> Result<Vec<Task>, DatabaseError> {
        let mut stmt = self.conn.prepare(
            "SELECT id, title, description, due_date, status, tags, \"order\", archived, created_at, updated_at
             FROM tasks WHERE archived = 0 ORDER BY \"order\" ASC"
        )?;
    ...
    /// Get all tasks including archived, ordered by order ASC
    pub fn get_all_tasks_including_archived(&self) -> Result<Vec<Task>, DatabaseError> {
        let mut stmt = self.conn.prepare(
            "SELECT id, title, description, due_date, status, tags, \"order\", archived, created_at, updated_at
             FROM tasks ORDER BY \"order\" ASC"
        )?;
    ...
    pub fn update_task(&self, task: &Task) -> Result<(), DatabaseError> {
        ...
        tx.execute(
            "UPDATE tasks SET title = ?1, description = ?2, due_date = ?3, 
             status = ?4, tags = ?5, \"order\" = ?6, archived = ?7, updated_at = ?8 WHERE id = ?9",
            rusqlite::params![
                task.title,
                task.description,
                task.due_date,
    ...
    pub fn get_all_notes(&self) -> Result<Vec<Note>, DatabaseError> {
        let mut stmt = self.conn.prepare(
            "SELECT id, title, content, tags, archived, created_at, updated_at
             FROM notes WHERE archived = 0 ORDER BY created_at DESC"
        )?;
    ...
    /// Get all notes including archived, ordered by created_at DESC
    pub fn get_all_notes_including_archived(&self) -> Result<Vec<Note>, DatabaseError> {
        let mut stmt = self.conn.prepare(
            "SELECT id, title, content, tags, archived, created_at, updated_at
    ...
    pub fn update_note(&self, note: &Note) -> Result<(), DatabaseError> {
        ...
        let tx = self.conn.unchecked_transaction()?;
        tx.execute(
            "UPDATE notes SET title = ?1, content = ?2, tags = ?3, archived = ?4, updated_at = ?5 WHERE id = ?6",
            rusqlite::params![
                note.title,
                note.content,
                note.tags,
    ...
    /// Get all journal entries ordered by date DESC (newest first)
    pub fn get_all_journals(&self) -> Result<Vec<JournalEntry>, DatabaseError> {
        let mut stmt = self.conn.prepare(
            "SELECT id, date, title, content, tags, archived, created_at, updated_at
             FROM journals WHERE archived = 0 ORDER BY date DESC, created_at DESC"
        )?;
        
        let journals = stmt.query_map([], |row| {
            Ok(JournalEntry {
                id: Some(row.get(0)?),
                date: row.get(1)?,
                title: row.get(2)?,
    ...
    /// Get all journal entries including archived, ordered by date DESC, created_at DESC
    pub fn get_all_journals_including_archived(&self) -> Result<Vec<JournalEntry>, DatabaseError> {
        let mut stmt = self.conn.prepare(
            "SELECT id, date, title, content, tags, archived, created_at, updated_at
             FROM journals ORDER BY date DESC, created_at DESC"
        )?;
    ...
    pub fn update_journal(&self, journal: &JournalEntry) -> Result<(), DatabaseError> {
        ...
        let tx = self.conn.unchecked_transaction()?;
        tx.execute(
            "UPDATE journals SET date = ?1, title = ?2, content = ?3, tags = ?4, archived = ?5, updated_at = ?6 WHERE id = ?7",
            rusqlite::params![
                journal.date,
                journal.title,
                journal.content,
